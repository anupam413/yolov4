...
class Transformer:
    ...
    def transform(self, image: ndarray, bbox: ndarray, label: ndarray):
        ...
        image: `~np.ndarray` of shape `(H, W, 3)`
            Image, the image mode is RGB or HUV, if there is no special description, the default RGB mode
        ...
        image, bbox, label:
            enhanced data
        """
        raise NotImplementedError("Image augmentation methods must be overridden")
...
class YoloAugmentation(Transformer):
    """ Yolo data augmenter for neural network training without mosaic augmentation """
...
class ColorAugmentation(Transformer):
    """ Yolo data augmenter used when neural network training uses mosaic augmentation """
...
class ToTensor(Transformer):
    """ Convert np.ndarray image to Tensor"""
    ...
    def transform(self, image: ndarray, bbox: ndarray = None, label: ndarray = None):
        """ Scale, center and convert the image to Tensor
